...
- `STOP` — app → device, ends the round without a press: both buttons are disarmed and the device returns to waiting-for-game-start. The app sends it when the question's time runs out and when it leaves a question for the start screen. Ignored if nothing is armed; a result already held from a press is still delivered.
